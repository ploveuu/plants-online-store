...
const BasketList = observer(() => {
	...
	return (
		<div style={{ display: 'flex', flexDirection: 'column', maxWidth: 800, margin: 'auto' }}>
			{basketList.map(basketItem => (
				<React.Fragment key={basketItem.id}>
					<Card className='d-flex flex-row mt-3 p-3' style={{ width: '100%' }} border='dark'>
						...
						<div
							...
							<div
								...
								<b>{basketItem.name}</b>
								<span>Цена: {basketItem.price} руб.</span>
								<span>
								...
								<span>
									<span>Сумма: </span>
									<b>{basketItem.price * basketItem.quantity}</b>
									<span> руб.</span>
								</span>
							</div>
			...
			<div
				...
				<span>
					<b>ИТОГО:</b> <span>{totalSum} руб.</span>
				</span>
